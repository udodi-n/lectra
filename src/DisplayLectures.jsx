...
const DisplayLectures = () => {
    ...
    return (
        <div className="mt-20 w-full min-h-screen flex flex-col justify-center items-center gap-7 py-10 overflow-y-auto text-white">
            <h2>Lectures - {date.toLocaleDateString()}</h2>
            {posts.map(post => {
                return
            })}
                        {posts.map(post => (
                                <div key={post.editId} className="aspect-1/1 relative sm:w-3/5 w-4/5 md:w-4/10 bg-white rounded-2xl overflow-hidden text-white">
                                <Banner 
